...
Centralizar toda la comunicación con el LLM en una única función de acceso es una decisión arquitectónica fundamental: garantiza que los parámetros de generación sean consistentes en las herramientas del servidor y convierte el mecanismos de \textit{fallback} en un servicio transversal sin duplicación de código. Esta decisión responde además a un problema concreto observado: las restricciones de cuota de plan gratuito de GitHub Models interrumpían con frecuencia sesisiones de modelado en curso, lo que exigía una estrategia de resiliencia completamente transparente para las herramientas de nivel superior. 
...
La decisión de descomponer la generación del diagrama en diferentes prompts, en lugar de usar un único prompt, es el núcleo de la estrategia de \textit{prompting} iterativo con validación humana en el bucle. Cada prompt tiene un contexto acotado y una tarea semánticamente bien definida, lo que reduce el espacio de respuestas posibles del LLM y mejora la conformidad con el esquema de salida esperado. Esta descomposición es también la que habilita la validación en cada paso, donde el usuario puede corregir cada elemento antes de que el LLM lo incorpore como contexto para la siguiente llamada, evitando que un error temprano se propague por todo el diagrama. 
...
La identificación de la estructura de participantes es la primera decisión del modelado y condiciona irrevocablemente toda la generación posterior: los pools y lanes establecidos aqui son el marco de referencia para las siguientes herramientas. La decisión de diseño central fue exigir una respuesta JSON con una distinción explicita de roles para actores externos, ya que sin esta distinción el LLM tendía a asignar flujos de mensaje de forma indiscriminada entre todos los participantes. 
...
Este prompt debe resolver simultáneamente dos problemas: elegir el tipo semántico correcto del elemento BPMN entre más de diez opciones posibles y determinar si el proceso ha concluido. Sin instrucciones de clasificación explícitas, los LLMs convergen por defecto al tipo genérico \texttt{task} para cualquier acción, ignorando la semántica de \texttt{userTask}, \texttt{serviceTask}, \texttt{sendTask} y los eventos intermedios, lo que produce diagramas semánticamente vacíos. Los bloques de reglas marcados con «OBLIGATORIO» y «NUNCA» surgen de sesiones de pruebas en las que formulaciones más suaves eran ignoradas consistentemente por el modelo. 
...
La decisión de general el XML de forma completamente programática, en lugar de delegar esa tarea al LLM, representa uno de los aportes de ingeniería más significativos del sistema. La alternativa de pedir al LLM el XML completo en una sola llamada fue explorada inicialmente y descartada porque los modelos generaban con frecuencia identificadores duplicados, referencias rotas en los \texttt{sequenceFlow} y coordenadas de posicionamiento incoherentes. La separación entre razonamiento semántico (qué elementos y en qué orden, tarea del LLM a través de las herramientas de generación iterativa) y construcción sintáctica es lo que permite que la herramienta produzxa diagramas  importables en bpmn.io de forma fiable.
...
Una vez generado el diagrama inicial, el usuario debe poder ajustarlo mediante instrucciones en lenguaje natural sin necesidad de regeneral el modelo desde cero: esta es la segunda mitad del paradigma de \textit{vibe modeling} guiado e híbrido. El reto de diseño fue determinar qué contexto necesita el LLM para aplicar una modificación puntual sin alterar el resto del diagrama; la experiencia mostró que proporcionar únicamente la instrucción y el XML resultaba insuficiente para cambios estructurales ya que el modelo alteraba elementos no mencionados, mientras que añadir el inventario explícito de elementos actuales (nombres, tipos y posición en el flujo) mejoró significativamente la precisión de las modificaciones. 
...
La herramienta de análisis PERVAL constituye la contribucción más directa de este TFM al problema de la alineación entre el valor diseñado en el proceso y el valor percibido por el cliente. La decisión de implementarla como una herramienta integrada en el servidor MCP y no como un módulo externo responde a un principio de integración: el usuario puede lanzar el análisis sobre el diagrama que acaba de generar o editar sin cambiar de herramienta ni exportar el modelo. La operacionalización de las cuatro dimensiones PERVAL en el contexto de tareas y entregas de procesos BPMN, es una adaptación original que fue refinada iterativamente a partir de los resultados de las pruebas.
...
Este módulo representa la respuesta de ingeniería a una limitación de los LLMs actuales: la imposibilidad de garantizar el formato de salida incluso cuando el prompt lo exige explícitamente. Durante el desarrollo, una proporción significativa de las respuesta de generación y refinamiento llegaban en formatos no parseables directamente. Sin este módulo, el sistema habría fallado en una fracción importante de las interacciones, lo que habría comprometido directamente la evaluación de aceptación con usuarios. 
...
Estas funciones surgieron como refactorizaciones de lógica que, en versiones tempranas del servidor, aparecía duplicadas en varios prompts. Su extracción a un módulo compartido aplica el principio DRY (\textit{Don't Repeat Yourself}) al nivel de construcción de prompts: centralizar aquí el tratamiento de los casos límite y garantiza que cualquier corrección futura se aplique uniformemente a todas las herramientas del sistema. 
