...
En el sistema propuesto se identifican dos actores externos, representados en la figura~\ref{fig:diagramaActores}:
...
\begin{itemize}
    ...
    \item \textbf{Proveedor de LLM}: servicio externo de modelos de lenguaje (\textit{GitHub Models}) utilizado por el sistema para interpretar las descripciones en lenguaje natural del usuario y generar o modificar los modelos BPMN, así como para realizar el análisis de valor PERVAL.
\end{itemize}

Los componentes internos de la solución ---la extensión sobre \textit{bpmn.io}, el servidor MCP y el módulo de análisis PERVAL--- se representan dentro de la frontera del sistema. En UML, los actores son entidades externas que interactúan con el sistema; por tanto, estos componentes no se modelan como actores sino como partes internas de la solución desarrollada.
...
\draw[thick, dashed] (2.0,-2.0) rectangle (7.0,1.8);
...
\node[align=center, font=\small] at (4.5,-0.7) {Módulo PERVAL};
...
El \textbf{Usuario} interactúa exclusivamente con la extensión cliente sobre \textit{bpmn.io}, sin tener visibilidad directa de las comunicaciones internas entre el servidor MCP y el \textbf{Proveedor de LLM}. Esta separación de responsabilidades da lugar a los diferentes diagramas de contexto que delimitan cada subsistema, presentados a continuación.
...
\node[extbox] (cliente) at (-5,0) {Extensión \textit{bpmn.io}\\(cliente)};
...
\node[extbox] (mcp) at (-5,0) {Servidor MCP};
