...
En este capítulo se aborda el análisis del problema que da origen a este Trabajo Fin de Máster: la integración de LLM en el modelado de procesos de negocio mediante \textit{bpmn.io}, junto con la incorporación de un módulo de análisis del valor percibido por el cliente basado en la clasificación PERVAL. Para ello, se ha realizado un modelado funcional del sistema mediante diagramas de actores, de contexto y de casos de uso, que permite delimitar el alcance del sistema y las interacciones previstas entre el usuario, la extensión sobre \textit{bpmn.io} y los proveedores de LLM. Asimismo, se ha analizado el marco legal y ético asociado al uso de modelos de lenguaje y al tratamiento de datos de los usuarios, y se han identificado y comparado las principales alternativas tecnológicas consideradas para abordar el problema, justificando las decisiones finalmente adoptadas. A partir de dicho análisis, se concreta la solución propuesta mediante la especificación de los requisitos funcionales y no funcionales del sistema y un conjunto de historias de usuario, así como la planificación del trabajo realizado, organizada en \textit{sprints} semanales siguiendo una metodología ágil.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Modelado funcional del sistema}
\label{sec:modeladoFuncional}

Con el objetivo de delimitar de forma precisa el alcance funcional del sistema desarrollado, se ha realizado un modelado funcional basado en diagramas UML simplificados, identificando los actores que interactúan con el sistema, los diagramas de contexto que delimitan los subsistemas considerados y sus relaciones con dichos actores, y los diagramas de casos de uso que describen las funcionalidades ofrecidas por cada subsistema.
...
\node[extbox] (llm) at (5,0) {LLM\\(\textit{GitHub Models} /\\\textit{Anthropic Claude})};
...
\begin{itemize}
    ...
    \item \textbf{Transparencia}: el sistema informa al usuario de qué proveedor de LLM se está utilizando en cada momento y permite seleccionar entre distintos modelos, de modo que el usuario es consciente de que las respuestas provienen de un sistema de inteligencia artificial generativa y no de un proceso determinista.
...
\begin{itemize}
    \item \textbf{Desarrollo de un editor BPMN propio desde cero}: ofrecería un control total sobre la representación interna del modelo y sobre la interfaz de usuario, pero implicaría un esfuerzo de desarrollo muy elevado, ajeno al objetivo principal del trabajo (la integración de LLM en el modelado de procesos), y un alto riesgo de introducir incompatibilidades con el estándar BPMN 2.0~\cite{omg2011bpmn}.
    \item \textbf{Herramientas de modelado de escritorio} (p.~ej. \textit{Camunda Modeler}, \textit{Bizagi Modeler}): proporcionan editores BPMN completos y maduros, pero al ser aplicaciones de escritorio cerradas resultan difíciles de extender con una interfaz conversacional integrada en el propio lienzo de modelado, y no exponen una API de extensión tan flexible como las soluciones basadas en web.
...
\begin{itemize}
    \item \textbf{Modelos locales de código abierto} (p.~ej. \textit{Llama}, \textit{Mistral}, ejecutados localmente mediante \textit{Ollama} u otras herramientas similares): evitarían el envío de datos a terceros y no tendrían coste de uso por petición, pero requerirían recursos de cómputo (GPU) no siempre disponibles, y su capacidad para generar XML BPMN válido de forma consistente es, en general, inferior a la de los modelos comerciales de mayor tamaño, como muestran trabajos previos sobre generación de modelos BPMN con LLM~\cite{kourani2025evaluating,bise2025bpmn}.
    \item \textbf{Un único proveedor comercial} (p.~ej. únicamente \textit{OpenAI} o únicamente \textit{Anthropic}): simplificaría la integración, pero introduciría un punto único de fallo: si el proveedor sufre una interrupción del servicio, agota la cuota disponible o deja de estar accesible, el sistema completo dejaría de funcionar.
    \item \textbf{\textit{GitHub Models} como proveedor principal, con \textit{Anthropic Claude} como proveedor alternativo}: \textit{GitHub Models} ofrece acceso gratuito (dentro de unos límites de uso) a una amplia variedad de modelos de distintos proveedores (\texttt{gpt-4.1}, \texttt{gpt-4o}, \texttt{gpt-4o-mini}, \texttt{gpt-4.1-mini}, \texttt{gpt-4.1-nano}, \texttt{Phi-4}, entre otros) a través de una API compatible con el formato de \textit{OpenAI}, lo que resulta idóneo para un proyecto académico. Adicionalmente, se incorpora soporte para la API de \textit{Anthropic Claude} como proveedor alternativo, configurable por el usuario.
\end{itemize}

Se optó por la tercera alternativa: \textbf{\textit{GitHub Models} como proveedor principal}, con un mecanismo de \textbf{selección automática de modelo de respaldo} (\textit{fallback}) entre los distintos modelos disponibles en \textit{GitHub Models} ante errores o límites de uso, y soporte adicional para \textbf{\textit{Anthropic Claude}} como proveedor alternativo configurable. Esta solución permite evaluar el comportamiento de distintos modelos frente a la misma tarea (generación de BPMN y análisis PERVAL) sin coste adicional, mejora la resiliencia del sistema ante fallos puntuales de un modelo concreto, y mantiene la arquitectura abierta a la incorporación de nuevos proveedores en el futuro.
...
\begin{enumerate}
    \item \textbf{Seguridad}: las claves de API de los proveedores de LLM (\texttt{GITHUB\_TOKEN} y, en su caso, la clave de \textit{Anthropic}) se almacenan exclusivamente como variables de entorno del servidor, sin exponerse en ningún momento al cliente.
    \item \textbf{Centralización de la lógica}: la construcción de \textit{prompts}, el \textit{parsing} robusto de las respuestas del LLM (función \texttt{parseLLMJson}), la reparación de XML BPMN truncado o malformado (función \texttt{cleanXML}) y el mecanismo de selección de modelo de respaldo (función \texttt{callOpenAI} con \textit{fallback} entre los modelos de \texttt{LLM\_MODELS}) se implementan una única vez en el servidor, en lugar de duplicarse en el cliente.
    \item \textbf{Extensibilidad}: el protocolo MCP define un formato estandarizado de herramientas (\textit{tools}) con esquemas de entrada y salida validados mediante \textit{Zod}, lo que facilita añadir nuevas funcionalidades --como el módulo de análisis PERVAL (herramienta \texttt{analyzePerval})-- como nuevas herramientas del mismo servidor, sin modificar la arquitectura general.
...
\hline
RF-06 & El sistema debe permitir al usuario seleccionar el proveedor de LLM (\textit{GitHub Models} / \textit{Anthropic Claude}) y, en su caso, el modelo concreto a utilizar. \\
\hline
RF-07 & El sistema debe seleccionar automáticamente un modelo de respaldo cuando el modelo principal no esté disponible o devuelva un error. \\
\hline
RF-08 & El sistema debe permitir analizar el valor percibido (PERVAL) de las tareas o entregables de un modelo BPMN, según las dimensiones emocional, social, de calidad/rendimiento y de precio~\cite{sweeney2001perval}. \\
\hline
RF-09 & El sistema debe permitir seleccionar el ámbito (tareas o entregables) y el actor sobre el que se realiza el análisis PERVAL. \\
\hline
RF-10 & El sistema debe presentar visualmente los resultados del análisis PERVAL (resumen por dimensión y valor general), acompañados de una justificación textual. \\
\hline
RF-11 & El sistema debe permitir seleccionar el idioma de la interfaz y de las respuestas generadas (español/inglés). \\
\hline
RF-12 & El sistema debe informar al usuario del estado de cada petición (cargando, completada, error) en todo momento. \\
\hline
...
\textit{Descripción}: como desarrollador, quiero analizar las alternativas tecnológicas disponibles para la generación de modelos BPMN mediante LLM y para el análisis PERVAL, con el fin de seleccionar el editor BPMN base, el o los proveedores de LLM, y la arquitectura de integración, y planificar el desarrollo del proyecto por \textit{sprints}.
...
\textit{Descripción}: como desarrollador, quiero configurar un entorno de desarrollo a partir del ejemplo \texttt{modeler} de \texttt{bpmn-js-examples}, de forma que disponga de un editor BPMN funcional sobre el que construir la extensión.
...
\textit{Descripción}: como desarrollador, quiero implementar un servidor \textit{backend} que siga la especificación de \textit{Model Context Protocol}~\cite{anthropic2024mcp}, de forma que actúe como intermediario seguro entre la extensión cliente y los proveedores de LLM.
...
\textit{Descripción}: como desarrollador, quiero integrar el servidor MCP con la API de \textit{GitHub Models}, de forma que pueda invocar distintos modelos de LLM (\texttt{gpt-4.1}, \texttt{gpt-4o}, etc.) para la generación de modelos BPMN y el análisis PERVAL.
...
\textit{Descripción}: como usuario, quiero describir un proceso de negocio en lenguaje natural y obtener automáticamente un modelo BPMN 2.0 que lo represente, de forma que pueda partir de un primer borrador del modelo sin tener que construirlo manualmente.
...
\textit{Descripción}: como usuario, quiero poder solicitar modificaciones sobre el modelo BPMN actual mediante mensajes en lenguaje natural, de forma que pueda refinar iterativamente el modelo a través de una conversación con el sistema.
...
\textit{Descripción}: como usuario, quiero que el sistema detecte y corrija automáticamente los errores habituales en el XML BPMN devuelto por el LLM (etiquetas sin cerrar, XML truncado), de forma que el modelo se cargue correctamente en el editor incluso cuando la respuesta del LLM no es perfecta.
...
\textbf{US08 -- Soporte de \textit{Anthropic Claude} como proveedor alternativo}

\textit{Descripción}: como usuario, quiero poder seleccionar \textit{Anthropic Claude} como proveedor de LLM alternativo a \textit{GitHub Models}, de forma que pueda comparar el comportamiento de distintos modelos ante la misma tarea.

\textit{Estimación}: 8h.

\textit{Tareas}:
\begin{itemize}
    \item Estudio de la API de \textit{Anthropic} (2h).
    \item Implementación del cliente de \textit{Anthropic Claude} y adaptación del formato de \textit{prompts} y respuestas (3h).
    \item Implementación del selector de proveedor de LLM en la interfaz (2h).
    \item Pruebas comparativas entre proveedores (1h).
\end{itemize}

\textbf{US09 -- Diseño del módulo de análisis PERVAL}

\textit{Descripción}: como desarrollador, quiero diseñar un módulo capaz de analizar el valor percibido por el cliente de las tareas o entregables de un modelo BPMN, según la clasificación de Sweeney y Soutar~\cite{sweeney2001perval}, de forma que el usuario pueda obtener información sobre el valor que aporta cada elemento del proceso.
...
\textbf{US10 -- Implementación de la herramienta \texttt{analyzePerval}}

\textit{Descripción}: como desarrollador, quiero implementar en el servidor MCP la herramienta de análisis PERVAL diseñada en US09, de forma que, dado un modelo BPMN y un conjunto de tareas, el sistema devuelva la clasificación por dimensiones, el resumen agregado y el valor general.
...
\textbf{US11 -- Visualización de los resultados del análisis PERVAL}

\textit{Descripción}: como usuario, quiero visualizar de forma clara los resultados del análisis PERVAL (resumen por dimensión, valor general y justificación de cada tarea), de forma que pueda interpretar fácilmente el valor percibido de cada elemento del proceso.
...
\textbf{US12 -- Diseño e implementación de la interfaz del panel conversacional}

\textit{Descripción}: como usuario, quiero disponer de un panel conversacional integrado en el editor con un diseño visual coherente y agradable, de forma que la interacción con el sistema resulte intuitiva y profesional.
...
\textbf{US13 -- Internacionalización de la interfaz y de los \textit{prompts}}

\textit{Descripción}: como usuario, quiero poder utilizar el sistema en español o en inglés, tanto en la interfaz como en las respuestas generadas por el LLM, de forma que pueda trabajar en el idioma de su preferencia.
...
\textbf{US14 -- Pruebas \textit{end-to-end} con \textit{Playwright}}

\textit{Descripción}: como desarrollador, quiero disponer de un conjunto de pruebas automatizadas \textit{end-to-end}, de forma que pueda verificar que los flujos principales del sistema (generación, edición conversacional y análisis PERVAL) funcionan correctamente en un navegador real.
...
\textbf{US15 -- Despliegue en \textit{Render} y documentación}

\textit{Descripción}: como desarrollador, quiero desplegar la extensión y el servidor MCP en \textit{Render}, de forma que el sistema sea accesible públicamente y pueda ser utilizado y evaluado fuera del entorno de desarrollo local.
...
\hline
Sprint 4 & 23/06/2026 -- 29/06/2026 & US08 (Soporte de \textit{Anthropic Claude}), US09 (Diseño del módulo PERVAL) & 18 \\
\hline
Sprint 5 & 30/06/2026 -- 06/07/2026 & US10 (Implementación de \texttt{analyzePerval}), US11 (Visualización de resultados PERVAL) & 20 \\
\hline
Sprint 6 & 07/07/2026 -- 13/07/2026 & US12 (Interfaz del panel conversacional), US13 (Internacionalización) & 18 \\
\hline
Sprint 7 & 14/07/2026 -- 20/07/2026 & US14 (Pruebas \textit{end-to-end}), US15 (Despliegue en \textit{Render} y documentación) & 15 \\
\hline
