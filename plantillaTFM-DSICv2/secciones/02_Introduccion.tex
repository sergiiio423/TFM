...
\begin{itemize}
    ...
    \item[$\bullet$] \textbf{Desarrollo:} este capítulo constituye el núcleo del trabajo y se articula en torno a tres bloques principales. El primero es el marco teórico, en el que se revisan los conceptos fundamentales necesarios para comprender el trabajo: el estándar BPMN y su aplicación al modelado de procesos de negocio, el funcionamiento y las capacidades de los LLMs para la generación y el análisis de texto, la clasificación PERVAL como marco para identificar el valor percibido por el cliente, y la herramienta bpmn.io como plataforma de modelado sobre la que se construye la extensión. El segundo bloque describe el diseño y la implementación de la herramienta propuesta, detallando la arquitectura de la extensión desarrollada sobre bpmn.io, el módulo de generación iterativa de modelos BPMN a partir de descripciones en lenguaje natural mediante un LLM y el módulo de identificación y clasificación del valor del cliente según PERVAL. El tercer bloque presenta el diseño del estudio empírico, incluyendo la definición del caso de estudio basado en un proceso de compra en línea de comercio electrónico, las iteraciones de modelado y análisis realizadas, y los criterios de comparación empleados para contrastar los resultados de la herramienta con los del analista experto.

    \item[$\bullet$] \textbf{Resultados:} en este capítulo se presentan y analizan los resultados obtenidos en el estudio empírico. Se comparan los modelos BPMN generados automáticamente con los elaborados por el analista experto, así como los valores identificados por la herramienta con los clasificados manualmente según la escala PERVAL. Se discute el grado de alineamiento entre ambos enfoques y se interpretan las diferencias encontradas.
    ...
    \item[$\bullet$] \textbf{Bibliografía:} se recopilan todas las fuentes de información consultadas durante la realización del TFM, incluyendo artículos de investigación, publicaciones académicas, documentación técnica y recursos digitales relevantes para el desarrollo del proyecto.

    \item[$\bullet$] \textbf{Anexos:} contienen información complementaria al cuerpo principal del documento, incluyendo material de apoyo al estudio empírico y otros recursos adicionales que facilitan la comprensión y la reproducibilidad del trabajo realizado.
\end{itemize}
